...
impl State {
    pub fn new() -> Self {
        ...
        entity_set.player.set_pos(vec2(480, 160));
        ...
        terrain::decode_chunk(vec2(2 * 16, 0), foreground.blocks_mut(), &[
            0x03, 0x05, 0xA1, 0x01, 0x54, 0x25, 0x00, 0x1E,
            //0x05, 0x0C, 0x01, 0x07, 0x0E, 0x20, 0x07, 0x0D, 0x00,
            0x06, 0x0C, 0x02, 0x07, 0x0E, 0x00,
            0x08, 0x7D, 0x40, 0x49, 0x59, 0x49, 0x59, 0x49
        ]);
        terrain::decode_chunk(vec2(3 * 16, 0), foreground.blocks_mut(), &[0x00, 0x10, 0x04, 0x01, 0xFD]);
        State {
            camera: vec2(0,60),
